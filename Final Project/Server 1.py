        ...
        try:
            if resource == "/":
                ...
                for item in list_species:
                    contents += f"<p><ul><li>{item}</li></ul></p>"
                ...
                contents = """
                        ...
                        """
                contents += f"<h2> Karyotype of the species: {specie_name[-1]}</h2>"
                ...
                contents = """
                    ...
                    """
                contents += f"<h2> Chromosome: {chromo_n[-1]} Species: {specie_n[0]}</h2>"
                ...
                self.send_response(200)
                    ...
                    if resource == '/geneInfo':
                        ...
                        contents += f"<p>The start: {ensembl_seq['start']}</p>"
                        contents += f"<p>The end: {ensembl_seq['end']}</p>"
                        contents += f"<p>The length of the gene's sequence is: {sequence.len()}</p>"
                        contents += f"<p>This gene is located in the chromosome: {ensembl_seq['seq_region_name']}</p>"
                    ...
                    if resource == "/geneCalc":
                        ...
                        contents += f"<p>The length of the sequence is: {sequence.len()}</p>"
                        for element in bases:
                            contents += f"<p>{element} : {sequence.count_base(element)} ({round(sequence.count_base(element) * (100 / sequence.len()), 2)}%)</p>"
                    self.send_response(200)
...
# EXCEPCIONES
# en list species: si el limit es mayor error, cambiar un poco las condiciones
# en karyotype: si la especie no existe error
# en length: si el chromosoma no existe pero la especie si error para evitar espacio en blanco
# indices
# chromo, start, end 	Return the names of the genes located in the chromosome "chromo" from the start to end positions
# gene list no funciona EXCP SI EL CHROMO NO EXISTE Y EL RESTO SI EROR, SI EL STARTING POINT NO EXISTE ERROR SI EL END POINT NO EXISTE ERROR